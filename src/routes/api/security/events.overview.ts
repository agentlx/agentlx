...
import { securityMachineEventsInputSchema } from "@/lib/security-monitoring";
import { requireScreenAccess } from "@/server/auth.server";
import { getEnterpriseSecurityMachineEventsOverview } from "@/server/edition.server";
...
export const Route = createFileRoute("/api/security/events/overview")({
  server: {
    handlers: {
      GET: async ({ request }) => {
        try {
          ...
          const input = securityMachineEventsInputSchema.parse(
            Object.fromEntries(new URL(request.url).searchParams),
          );
          return jsonResponse(
            await getEnterpriseSecurityMachineEventsOverview({
              ...input,
          ...
          return jsonError(
            publicErrorMessage(error, "Falha ao carregar eventos da maquina."),
